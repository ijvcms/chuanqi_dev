...
init([]) ->
	...
	{ok, LotteryState1} = lottery_lib:init(LotteryState),
	lottery_coin_lib:init(LotteryState1).
%%--------------------------------------------------------------------
%% @private
%% @doc
...
handle_apply_sync_return({ok, Reply}, _Mfa, State) ->
	{reply, {ok, Reply}, State};
handle_apply_sync_return({stop, Reason, State}, _Mfa, State) ->
	{stop, Reason, State};
...
handle_apply_async_return(ok, _Mfa, State) ->
	{noreply, State};
handle_apply_async_return({stop, Reason, State}, _Mfa, State) ->
	{stop, Reason, State};
