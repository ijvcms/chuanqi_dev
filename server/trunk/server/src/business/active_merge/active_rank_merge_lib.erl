...
get_rank_list(PlayerState, Type, BegionTime, EndTime) ->
	...
	case BegionTime > CurTime of
		...
		_ ->
			%% 排名列表
			RankList = rank_lib:get_rank_list_active_merge_type(Type, false),
			...
			MyLv =
				case EndTime > CurTime of
					true ->
						%% 如果活动还未结束就调用当前的值
						active_service_merge_lib:get_type_now_value(PlayerState, Type);
					_ ->
						%% 如果活动已经结束，就调用结束的值
						active_service_merge_lib:get_type_value(PlayerId, Type)
				end,
...
checke_acitve_over(PlayerState) ->
	F = fun(X) ->
		ActiveServerTypeConf = active_service_merge_type_config:get(X),
		case ActiveServerTypeConf#active_service_merge_type_conf.receive_state of
			?RECEIVE_STATE_2 ->
				%% 获取活动列表
				[checke_active_service(PlayerState, M) || M <- active_service_merge_config:get_list_conf(),
					M#active_service_merge_conf.rank =:= 0 andalso M#active_service_merge_conf.type =:= X];
			_ ->
				skip
		end
	end,
	[F(X) || X <- active_service_merge_type_config:get_list(), active_service_merge_lib:is_over(X)].

%% 发送玩家的开服活动信息
checke_active_service(PlayerState, ActiveServerConf) when is_record(PlayerState, player_state) ->
	case active_service_merge_lib:check_active(PlayerState, ActiveServerConf, ActiveServerConf#active_service_merge_conf.type) of
		0 ->
...
send_active_goods(PlayerId, Career, ActiveServerConf) ->
	...
	case player_active_service_merge_cache:select_row({PlayerId, ActiveServiceId}) of
		null ->
			...
			GoodsList = active_service_merge_lib:get_goods_list(Career, RewardFa, RewardDao, RewardZhan),
