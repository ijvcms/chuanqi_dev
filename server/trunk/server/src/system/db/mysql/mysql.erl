...
fetch2(Query) ->
	...
	undefined ->
	    {error, not_in_transaction};
	State ->
	    mysql_conn:fetch_local2(State, Query)
    end.
...
%%   query_result()
fetch2(PoolId, Query) ->
    fetch2(PoolId, Query, undefined).

fetch2(PoolId, Query, Timeout) -> 
    case get(?STATE_VAR) of
	undefined ->
	    call_server({fetch2, PoolId, Query}, Timeout);
	State ->
	    mysql_conn:fetch_local2(State, Query)
    end.
...
handle_call({fetch2, PoolId, Query}, From, State) ->
    fetch_queries(PoolId, From, State, Query);
