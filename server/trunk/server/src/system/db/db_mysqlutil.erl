	...
	case erlang:is_tuple(Sql) of
		false ->
			case mysql:fetch(Db_pool, Sql) of
				{updated, {_, _, _, R, _}} -> R;
				{data, {_, _, R, _, _}} -> R; %% zhengsiying(2016.02.25)
				{error, {_, _, _, _, Reason}} ->
					?ERR("mysql sql:~ts err:~p",[Sql,Reason]),
...
execute2(Sql) ->
	case erlang:is_tuple(Sql) of
		false ->
			case mysql:fetch2(?DB_POOL, Sql) of
				{updated, {_, _, _, R, _}} -> R;
				{data, {_, _, R, _, _}} -> R; %% zhengsiying(2016.02.25)
				{error, {_, _, _, _, Reason}} ->
					?ERR("mysql sql:~ts err:~p",[Sql,Reason]),
		...
		_ ->
			{NewSql, Server} = Sql,
			case mysql:fetch2(Server, ?DB_POOL, NewSql, undefined) of
				{updated, {_, _, _, R, _}} -> R;
				{data, {_, _, R, _, _}} -> R; %% zhengsiying(2016.02.25)
				{error, {_, _, _, _, Reason}} -> mysql_halt([NewSql, Reason])
			end
...
%% 未找到时返回null
get_one({Sql, DbPool}) ->
	case mysql:fetch(DbPool, Sql) of
		{data, {_, _, [], _, _}} -> [];
		{data, {_, _, [[R]], _, _}} -> R;
		{error, {_, _, _, _, Reason}} -> mysql_halt([Sql, Reason])
	end.
...
get_row({Sql, DbPool}) ->
	case erlang:is_tuple(Sql) of
		...
		_ ->
			{NewSql, Server} = Sql,
			case mysql:fetch(Server, DbPool, NewSql, undefined) of
				{data, {_, _, [], _, _}} -> [];
				{data, {_, _, [R], _, _}} -> R;
				{error, {_, _, _, _, Reason}} -> mysql_halt([NewSql, Reason]);
				_ -> mysql_halt([Sql, "db_mysqlutil:get_row error"])
...
make_update_sql(Table_name, Field, Data, Key, Value) ->
	...
	lists:concat(["update `", Table_name, "` set ", L, " where ", Key, "= '", util_data:to_list(Value), "'"]).
...
%% 	end;
	L = ["`", util_data:to_list(F), "`='", get_sql_val(D), "'"],
	make_conn_sql(T1, T2, L);
...
%% 	end.
	L1 = L ++ [",`", util_data:to_list(F), "`='", get_sql_val(D), "'"],
	make_conn_sql(T1, T2, L1).

get_sql_val(Val) ->
	case is_binary(Val) orelse is_list(Val) of
		true -> re:replace(util_data:to_list(Val), "'", "''", [global, {return, list}]);
		_ -> util_data:to_list(Val)
	end.
