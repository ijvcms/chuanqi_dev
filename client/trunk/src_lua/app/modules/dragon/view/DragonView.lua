...
local DragonView = DragonView or class("DragonView", BaseView)

function DragonView:ctor(winTag,data,winconfig)
	--self:creatPillar()\
	...
	DragonView.super.ctor(self,winTag,data,winconfig)
...
function DragonView:setViewInfo(data)
	if data == nil then 
		data = configHelper:getDarkHouseGoods(2)
	end
...
function DragonView:open()
	self:setViewInfo(nil)
end

function DragonView:close()
end
