...
function FacePage:buildFaces()
	...
	for i = 1, #self._faces do
		local key = self._faces[i].key
		ArmatureManager:getInstance():loadEffect(key)
        local item = ccs.Armature:create(key)
        item.name = key
...
function FacePage:setVisible(visible)
	getmetatable(self).setVisible(self, visible)
	if visible then
		for i = 1, #self._items do
			self._items[i]:getAnimation():play("effect")
		end
	else
		for i = 1, #self._items do
			self._items[i]:getAnimation():stop()
		end
	end
...
function FacePage:onTouchHandler(event)
	...
	elseif "ended" == event.name then
		if not self.hasMoving then
            ...
            self:handleTouchHasContain(self._items, nodeSpacePoint, handler(self, self.onClickItem))
		end
	end
	return true
end
...
function FacePage:Destory()
	for i = 1, #self._items do
		self._items[i]:getAnimation():stop()
		ArmatureManager:getInstance():unloadEffect(self._faces[i].key)
	end
end
