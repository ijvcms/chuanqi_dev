...
local ServerDarkWin = class("ServerDarkWin", BaseView)
function ServerDarkWin:ctor(winTag,data,winconfig)
    self.bg =  cc.LayerColor:create(cc.c4b(0,0,0,100))
        self.bg:setContentSize(display.width, display.height)
        self:addChild(self.bg)
    ServerDarkWin.super.ctor(self,winTag,data,winconfig)
    self.root:setPosition((display.width-960)/2,(display.height-640)/2)
    --self:creatPillar()
	-- if isShowCloseBtn then
	-- end
	-- self.root = cc.uiloader:load("resui/serverBossItem2.ExportJson") --serverDarkWin
    ...
    self.closeBtn:addNodeEventListener(cc.NODE_TOUCH_EVENT, function(event)
        ...
        elseif event.name == "ended" then
            ...
            GlobalWinManger:closeWin(WinName.SERVERDARKWIN)
            --GlobalWinManger:closeWin(WinName.INTERSERVISE)
        end
        return true
    end)
    ...
    --             enterTxt = "确定",
    --             backTxt= "取消",
    --             tipTxt = configHelper:getRuleByKey(22),
    --             tipShowMid = true,
    --             hideBackBtn = true,
    --           })
    --     end     
...
function ServerDarkWin:open(datas)
    self:setVisible(true)
    self.config = configHelper:getDarkHouseGoods(8)
...
function ServerDarkWin:close()
    ServerDarkWin.super.close(self)
end

--清理界面
function ServerDarkWin:destory()
