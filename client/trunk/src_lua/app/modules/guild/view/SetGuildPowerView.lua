...
function SetGuildPowerView:ctor(isCorps)
	...
	if self.isCorps then
		richStr = "<font color='0xffd3af' size='20' >1.入团条件可以设置战斗力,仅限数字输入<br gap='10'/>2.入团最低要求25000战斗力</font>"
	    ruleY = 60
	end
	local strRT = SuperRichText.new(richStr)
    -- strRT:setAnchorPoint(0,1)
    strRT:setPosition(15, ruleY)
    bg:addChild(strRT)
    ...
    if self.isCorps then
    	...
    	GameNet:sendMsgToSocket(37002, {legion_id = corpsInfo.legion_id})
    else
...
function SetGuildPowerView:registerEvent()
    ...
    if self.isCorps then
    	self.registerEventId[1] = GlobalEventSystem:addEventListener(CorpsEvent.REQ_ENTER_CORPS_COND,onGetCondition)
    else
...
function SetGuildPowerView:onModifyClick()
	...
	if self.isCorps then
		local newFight = tonumber(self.inputLab:getText(), 10)
		if newFight < 25000 then
			GlobalEventSystem:dispatchEvent(GlobalEvent.SHOW_SYSTIPS, "入团最低要求25000战斗力")
			return
		end
		GameNet:sendMsgToSocket(37003, {fight = newFight})
	else
