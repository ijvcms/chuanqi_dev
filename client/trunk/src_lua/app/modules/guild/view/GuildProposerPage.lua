...
local GuildProposerPage = class("GuildProposerPage", GuildBasePage)

function GuildProposerPage:ctor()
...
function GuildProposerPage:createPages(pageCount)
	...
	--请求第一页列表
	GameNet:sendMsgToSocket(17008, {min_value=1,max_value=GMP_infoPerPage})
end

function GuildProposerPage:createList(listData,min,max)
...
function GuildProposerPage:onNextPage(isRight)
	...
	if self.pageLayers[pageIndex] then
		...
		if self.pageFirstLoad[pageIndex] == nil then
			--请求下一页
			GameNet:sendMsgToSocket(17008, {min_value=(self.curPageIndex-1)*GMP_infoPerPage+1,max_value=(self.curPageIndex-1)*GMP_infoPerPage+GMP_infoPerPage})
		end
		self.labPage:setString(self.curPageIndex.."/"..(self.maxPage or 1))
	end
end

function GuildProposerPage:onNodeEvent(data)
	if data.name == "cleanup" then
        self:unregisterEvent()
    end
end

function GuildProposerPage:registerEvent()
	self.registerEventId = {}
    --取得一页申请人列表
    local function onGetListInfo(data)
    	self:createList(data.data.apply_guild_info,data.data.min_value)
    end
    self.registerEventId[1] = GlobalEventSystem:addEventListener(GuildEvent.REQ_GUILD_PROPOSER_LISTINFO,onGetListInfo)
    ...
    self.registerEventId[2] = GlobalEventSystem:addEventListener(GuildEvent.REQ_GUILD_PROPOSER_COUNT,onGetProposerCount)
    ...
    self.registerEventId[3] = GlobalEventSystem:addEventListener(GuildEvent.PROPOSER_AGREE,onAgree)
    ...
    self.registerEventId[4] = GlobalEventSystem:addEventListener(GuildEvent.PROPOSER_REFUSE,onRefuse)

    --申请人已被处理
    local function onProposerHandle(data)
    	--yjc
    end
    self.registerEventId[5] = GlobalEventSystem:addEventListener(GuildEvent.PROPOSER_HANDLE,onProposerHandle)
end

function GuildProposerPage:unregisterEvent()
...
function GuildProposerPage:onBtnClick(index)
	...
	if index == 1 then 			--申请设置
	    ...
	    local node = SetGuildPowerView.new()
	...
	elseif index == 3 then 		--拒绝
		...
		GameNet:sendMsgToSocket(17009,data)
...
function GuildProposerPage:onShow()
	if self.pageLayers then
		for k,v in pairs(self.pageLayers) do
			v:removeSelf()
		end
	...
	--请求有多少申请人
	GameNet:sendMsgToSocket(17007,{})
	--
	self.haveGetCount = false
end

return GuildProposerPage