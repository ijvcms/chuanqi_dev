...
local GuildMemberPage = class("GuildMemberPage", GuildBasePage)

function GuildMemberPage:ctor()
...
function GuildMemberPage:createPages(pageCount)
	...
	--请求第一页列表
	GameNet:sendMsgToSocket(17012, {min_value=1,max_value=GMP_infoPerPage})
end

function GuildMemberPage:createList(listData,min,max)
	...
	if self.pageFirstLoad[page] == nil then 	--第一次取得该页数据
		...
		for i=1,#listData do
			local memberInfo = MemberListInfo.new(listData[i],i)
...
function GuildMemberPage:onNextPage(isRight)
	...
	if self.pageLayers[pageIndex] then
		...
		if self.pageFirstLoad[pageIndex] == nil then
			--请求下一页
			GameNet:sendMsgToSocket(17012, {min_value=(self.curPageIndex-1)*GMP_infoPerPage+1,max_value=(self.curPageIndex-1)*GMP_infoPerPage+GMP_infoPerPage})
		end
		self.labPage:setString(self.curPageIndex.."/"..(self.maxPage or 1))
	end
end

function GuildMemberPage:onNodeEvent(data)
	if data.name == "cleanup" then
        self:unregisterEvent()
    end
end

function GuildMemberPage:registerEvent()
	self.registerEventId = {}
    --取得一页成员列表
    local function onGetListInfo(data)
    	self:createList(data.data.guild_member_info_list,data.data.min_value)
    end
    self.registerEventId[1] = GlobalEventSystem:addEventListener(GuildEvent.REQ_GUILD_MEMBER_LISTINFO,onGetListInfo)
    ...
    self.registerEventId[2] = GlobalEventSystem:addEventListener(GuildEvent.REQ_GUILD_MEMBER_COUNT,onGetMemberCount)
		...
		for k,v in pairs(self.pageLayers) do
			...
			for i=1,#childs do
				if childs[i].data.player_id == data.data.player_id then
					...
					childs[i].labs[2]:setString(GuildPosition[data.data.position])
					local playerName = childs[i].data.player_name
					GlobalEventSystem:dispatchEvent(GlobalEvent.GET_ERROR_CODE,ErrorCodeInfoFormat(ERR_COMMON_SUCCESS,playerName.."被任命为"..GuildPosition[data.data.position]))
					return 
				end
			end
		end
    end
    self.registerEventId[3] = GlobalEventSystem:addEventListener(GuildEvent.REQ_APPOINT_MEMBER,onMemberPositionChange)
	...
	self.registerEventId[4] = GlobalEventSystem:addEventListener(GuildEvent.REQ_REJECT_MEMBER,onReqRejectMember)
end

function GuildMemberPage:unregisterEvent()
    ...
    if self.RoleGuildInfoChangeEvent then
    	GlobalEventSystem:removeEventListenerByHandle(self.RoleGuildInfoChangeEvent)
    	self.RoleGuildInfoChangeEvent = nil
    end
end

function GuildMemberPage:onBtnClick(index)
	...
	if index == 1 then 		--更多操作按钮
	    ...
	    if guildInfo.position == 1 then 			--会长
	    	node = MoreOperateList.new({1,2,3,4,5},self.curInfo.data,{[3]=handler(self,self.onTransferLeader)})
	    elseif guildInfo.position == 2 then 		--副会长
	    	node = MoreOperateList.new({1,2,4,5},self.curInfo.data)
	    elseif guildInfo.position == 3 or guildInfo.position==4 then 		--精英和会员
	    	
	    	node = MoreOperateList.new({1,2},self.curInfo.data)
	    else
	    	node = MoreOperateList.new({1,2},self.curInfo.data)
	    end
	    if node then
...
function GuildMemberPage:onTransferLeader()
	local roleManager = RoleManager:getInstance()
    local guildInfo = roleManager.guildInfo
    --不是会长,没有权利
    ...
    GameNet:sendMsgToSocket(17019, {player_id = self.curInfo.data.player_id, position = 1})
    --主角帮会职位信息改变
    if not self.RoleGuildInfoChangeEvent then
    	local function onRoleGuildInfoChange()
    		if self.RoleGuildInfoChangeEvent then
		    	GlobalEventSystem:removeEventListenerByHandle(self.RoleGuildInfoChangeEvent)
		    	self.RoleGuildInfoChangeEvent = nil
		    end
		    ...
		    for k,v in pairs(self.pageLayers) do
				...
				for i=1,#childs do
					if childs[i].data.player_id == roleInfo.player_id then
						childs[i].data.position = guildInfo.position
						childs[i].labs[2]:setString(GuildPosition[guildInfo.position])
						return 
					end
				end
			end
	    end
	    self.RoleGuildInfoChangeEvent = GlobalEventSystem:addEventListener(GuildEvent.ROLE_GUILD_INFO_CHANGE,onRoleGuildInfoChange)
    end
end

function GuildMemberPage:onShow()
